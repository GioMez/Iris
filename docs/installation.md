...
Use the source at the exact Git tag **`R1.0.0`**. From a directory where you want
to keep the installation:

```sh
git clone --branch R1.0.0 --depth 1 https://github.com/GioMez/WebTeX.git iris-1.0.0
cd iris-1.0.0
```

Or download **`iris-1.0.0.tar.gz`** and **`SHA256SUMS`** from the
[R1.0.0 release](https://github.com/GioMez/WebTeX/releases/tag/R1.0.0).
...
one `iris-1.0.0/` directory:

```sh
tar -xzf iris-1.0.0.tar.gz
cd iris-1.0.0
```
...
image **`localhost/iris:1.0.0`** from the tagged or extracted source. You do not
...
The 1.0.0 distribution supplies one compiler-free Iris image recipe and the
...
Iris 1.0.0 initializes **database schema 2** and reopens a matching schema.
