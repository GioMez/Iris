...
test("compilation is bounded in both rate and simultaneity", () => {
  ...
  assert.ok(slot < compile.indexOf("createBuildOutput("), "no build row may exist before the slot is held");
  assert.ok(slot < compile.indexOf("writeProjectFile(stagingDir"), "no staging tree may exist before the slot is held");
...
test("the project row is touched once per interval, not once per room per flush", () => {
  ...
  assert.doesNotMatch(persist, /db\.query/, "the flush path must not write to the database directly");
...
test("retention is an owner control, and losing the role closes it in place", () => {
  ...
  // schedulePersist only runs when autosave is on and a file is dirty, so it
  // would drop a settings-only change entirely.
...
test("the panel shows what the server stored, not what was asked for", () => {
  ...
  const persist = projects.slice(projects.indexOf("function persistCurrent"), projects.indexOf("async function compileCurrent"));
  // Read before the request: by the time the queued save runs, the panel may
  // have moved on, and this save must carry the values it was scheduled with.
  assert.ok(
    persist.indexOf("window.IrisApp.pendingRetention()") < persist.indexOf("const operation = async"),
    "the pending values must be captured before the operation is queued"
  );
  assert.match(persist, /\.\.\.\(retention \? \{ retention \} : \{\}\)/);
  assert.match(persist, /window\.IrisApp\.applyRetention\(out\.retention\)/);
  // A save landing after the project was reopened must not write into the new one.
  assert.match(persist, /if \(!staleReopen && out && out\.retention/);
});
